    ...
    <div class="bg-white padding-top padding-bottom">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="180px" class="demo-ruleForm" style="width:500px" >
        ...
        <el-form-item label="授权委托书(单位介绍信)">
          ...
          <div v-if="type != '1'">
            <a href="@/assets/授权委托书模板.docx" target="_blank" download="授权书模板下载">
              授权书模板下载 
            </a>
            <a href="https://docs.qq.com/sheet/DRnRQWFppb0xuS090?tab=BB08J2" download="授权书模板.docx">点击此处下载模板</a>
          </div>
        </el-form-item>
        <el-form-item>
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$post( "/api/user/userAuthenticationByCompany", this.ruleForm ).then(response => { 
            if (response.code == 0) { 
                _this.$message({ message: '添加成功', type: 'success', duration: 1500, onClose: () => {  _this.$router.go(-1); }})
              }else{ 
