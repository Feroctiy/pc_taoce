  ...
  <div class="main_width stepBox">
    ...
    <el-card class="box-card margin-bottom">
      ...
      <div>
        <el-table
          ...
          row-key="2c9a538c72d74efd0172db00d7bd002e"
        >
    ...
    <el-card class="box-card margin-bottom">
      <div class="info">
        ...
        <span
          v-if="!checked && goodsDetail.goodsCheckitemEntityList.length == multipleSelection.length"
        >￥{{goodsDetail.goodsPrice}}</span>
        <span
          v-else-if="checked  && goodsDetail.goodsCheckitemEntityList.length == multipleSelection.length "
        >￥{{goodsDetail.goodsPrice + goodsDetail.jjjg}}</span>
...
export default {
  ...
  mounted() {
    ...
    //默认全选
    if(this.param.fwItemIds){
      this.$refs.multipleTable.toggleRowSelection(
        goodsDetail.goodsCheckitemEntityList[2],
        true
      ); 
    }else{
      this.$nextTick(() => {
        for (let i = 0; i < this.goodsDetail.goodsCheckitemEntityList.length; i++) {
          this.$refs.multipleTable.toggleRowSelection(
            this.goodsDetail.goodsCheckitemEntityList[i],
            true
          );
        }
      });
