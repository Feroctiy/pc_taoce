<template>
    <div class="max_width bg-white padding" id="shopJoin">
            ...
            <el-form :inline="true" :label-position="labelPosition" label-width="80px" :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="相关附件上传" style="width: calc(100% - 120px);">
                    <div>
                        ...
                        <el-upload class="upload-demo" :before-upload="beforeAvatarUpload" :on-success="handleAvatarSuccess"
                            :on-preview="handlePreview" :on-remove="handleRemove" :before-remove="beforeRemove"  multiple
                            :limit="1" :action="uploadUrl" :show-file-list="true" :on-exceed="handleExceed" :file-list="fileList">
            ...
            <el-card class="box-card margin-bottom" v-for="(item,index) in joinCertificateFromList" :key="index">
                ...
                <div v-if="item.isHide">
                    <el-form :label-position="labelPosition" label-width="80px" :model="form">
                        ...
                        <el-form-item label="备注说明" style="width: calc(100% - 120px);">
                            <el-input type="textarea" v-model="item.remark" placeholder="请输入备注说明" maxlength="500" show-word-limit rows="4"></el-input>
                        </el-form-item>
                        <el-form-item label="相关附件上传" style="width: calc(100% - 120px);">
                            <div>
                                <span>
        ...
        <div v-if="active == '3'">
            ...
            <el-form :label-position="labelPosition" label-width="80px" :model="info" :rules="inforules" ref="info" >
                ...
                <el-form-item label="验证码" prop="yzm">
                    <el-input v-model="info.yzm" placeholder="请输入验证码" style="width:300px;display:inline-block;"
...
export default {
  ...
  data() {
    ...
    var mobileValidator = (rule, value, callback) => {
      if (!validateMobile(value)) { callback(new Error("手机号格式错误")); } else { callback(); }
    };
    var valiIcon = (rule, value, callback) => { 
      if (!this.hasFmt) { callback(new Error("请上传logo")); } else { callback(); }
    };
    return {
      hasFmt: false,
      disabled:false,
      ...
      fileList1: [],
      rules: {
        mc: [{ required: true, message: "请输入机构或代理商名称", trigger: "blur" }],
        creditCode: [{ required: true, message: "请输入统一社会信用代码", trigger: "blur" }],
        addr: [{ required: true, message: "请输入注册地址或身份证地址", trigger: "blur" }],
        legalPerson: [{ required: true, message: "请输入法定代表人", trigger: "blur" }],
        isNationLevel: [{ required: true, message: "请选择是否为国家中心", trigger: "change" }],
        isProvinceLevel: [{ required: true, message: "请选择是否为省中心", trigger: "change" }],
        mtype: [{ required: true, message: "请选择类型", trigger: "change" }],
        value: [{ required: true, message: "请选择行政区划", trigger: "change" }],
        intro: [{ required: true, message: "请输入简介", trigger: "blur" }],
        businessScope: [{ required: true, message: "请输入机构业务范围", trigger: "blur" }],
        imageUrl: [{ required: true, validator: valiIcon }]
      },
      inforules: {
        contactUser: [{ required: true, message: "请输入机构入驻联系人", trigger: "blur" }],
        qq: [{ required: true, message: "请输入机构入驻联系人QQ号", trigger: "blur" }],
        email: [
          { required: true, message: "请输入机构入驻联系人邮箱", trigger: "blur" },
          { validator: emailValidator, trigger: "blur" }
        ],
        contactPhone: [
          { required: true, message: "请输入机构入驻联系人手机号", trigger: "blur" },
      ...
      joinCertificateFromList: [ { ceImgList: [], abilityFileList: [], isHide:true } ],
  ...
  created() {
    ...
    cityList.forEach(function(value, index) {
      ...
      value.ssqdmEntityList.forEach(function(v, i) {
        _this.options[index].children.push({ value: v.dm, label: v.name, children: []  });
        v.ssqdmEntityList.forEach(function(x, y) {
          _this.options[index].children[i].children.push({ value: x.dm, label: x.name });
        });
      });
    });
  ...
  methods: {
    ...
    three() {
      ...
      for (let i = 0; i < this.joinCertificateFromList.length; i++) {
        ...
        if (isEmpty(this.joinCertificateFromList[i].ceImg)) {
          this.$message.error("资质证书"+(i+1)+": 请上传检验检测资质证书");
          return;
        }
    ...
    addCertificate() {
      this.joinCertificateFromList.push({ ceImgList: [], abilityFileList: [],isHide:true });
    },
